...
class CaptureTools(BaseToolSet):
        ...
        try:
            ...
            return ToolResult(
                content=[
                    ...
                    TextContent(type="text", text=f"📸 Screenshot of {target}{saved_msg}")
                ]
            )
    ...
    async def _get_text(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                content=[TextContent(type="text", text=f"📄 Text from {source}:\n\n{text}")]
            )
        except Exception as e:
            return ToolResult(
                content=[TextContent(type="text", text=f"❌ Text extraction failed: {str(e)}")],
                is_error=True
            )
    ...
    async def _get_html(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                content=[TextContent(type="text", text=f"🔖 HTML from {source}:\n\n```html\n{html}\n```")]
            )
        except Exception as e:
            return ToolResult(
                content=[TextContent(type="text", text=f"❌ HTML extraction failed: {str(e)}")],
                is_error=True
            )
    ...
    async def _get_console_messages(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if not messages:
                return ToolResult(
                    content=[TextContent(type="text", text="📋 No console messages")]
                )
            
            messages_text = "\n".join(messages)
            return ToolResult(
                content=[TextContent(type="text", text=f"📋 Console messages:\n\n{messages_text}")]
            )
        except Exception as e:
            return ToolResult(
                content=[TextContent(type="text", text=f"❌ Console messages failed: {str(e)}")],
                is_error=True
            )
